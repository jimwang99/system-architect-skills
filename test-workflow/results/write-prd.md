...
> RED baselines captured before any `write-prd/SKILL.md` exists. Each subagent got the scenario Prompt verbatim plus only its cwd and that validator tools exist at the worktree path — no skill, no bootstrap guidance. Commit pins repo HEAD `990b815`, which contains the scenario files and validators these entries prove something about (fixtures live in a scratch dir and are not committed). Note: agents 01 and 02 read the workflow specs and (in 01's case) the scenario file on their own; that contamination made them *more* compliant than a naive agent, so recorded violations are conservative.
